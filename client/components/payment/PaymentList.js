...
import {useGetPaymentsQuery,useVerifyPaymentMutation,useDeclinePaymentMutation} from "@/api/api-slice";
...
function PaymentList() {
  const {data,isError,isLoading,isSuccess} = useGetPaymentsQuery();
  ...
  const handleDecline=async (id)=>{
    const response = await declinePayment(id);
    console.log(response)
          ...
          <table className="w-full min-w-max table-auto text-left px-20">
              ...
              <tr>
                {TABLE_HEAD.map((head) => (
                  ...
                  >
                    <Typography
                      color="#334155"
                      className="font-normal text-[#334155] leading-none opacity-70 text-md"
                    >
                      {head}
                    </Typography>
                  </th>
            ...
            <tbody>
              {data.map((payment, index) => (
                <tr
                  key={payment._id}
                  // onClick={() => router.push(`/admin/teacher/${id}`)}
                ...
                >
                  ...
                  <td className="flex">
                    <Tooltip content="Accept Payment">
                      <IconButton className="px-auto" variant="text" onClick={()=>{handleVerify(payment._id)}}>
                        <FaCheck color="green" size={20} />{" "}
                      </IconButton>
                    </Tooltip>
                    <Tooltip content="Reject Payment" >
                      <IconButton className="px-auto" variant="text" onClick={()=>{handleDecline(payment._id)} }>
